...
class LayoutD extends StatelessWidget {
  ...
  const LayoutD({Key? key, this.start = 0.0}) : super(key: key);
  ...
  Positioned _buildContainer({required double gap, required double size, required double left, required double top}) {
    return Positioned(
        left: left,
        top: top,
        width: size,
  ...
  Positioned _buildAnimatedContainer({required double gap, required TimelineValue<_P> value}) {
    ...
    return Positioned(
        ...
        child: Container(
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.all(Radius.circular(value.get(_P.borderRadius1))),
          ),
          child: Stack(
            children: [
              Positioned.fill(
...
TimelineTween<_P> _createTween(double size, double gap) {
  ...
  open.addSubsequentScene(duration: 300.milliseconds).animate(_P.videoOpacity, tween: (0.0).tweenTo(1.0));
  ...
  final close = open
      ...
      .animate(_P.shiftY, tween: (boxSize * 2).tweenTo(0.0), shiftBegin: 0.milliseconds, shiftEnd: 300.milliseconds);
