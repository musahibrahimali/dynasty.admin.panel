...
class LayoutB extends StatelessWidget {
  ...
  const LayoutB({Key? key, this.start = 0.0}) : super(key: key);
                        ...
                        child: Container(
                          decoration: BoxDecoration(
                            ...
                            borderRadius: BorderRadius.only(
                              ...
                              topRight: Radius.circular(value.get(_P.rightBorderRadius1)),
                              bottomLeft: Radius.circular(gap),
                              bottomRight: Radius.circular(value.get(_P.rightBorderRadius1)),
                            ),
                          ),
                        )),
                    Positioned(
                        left: value.get<double>(_P.left1) + value.get<double>(_P.width1) + 2 * gap,
                        right: 2 * gap,
                        ...
                        child: ClipRect(
                          child: OverflowBox(
                            ...
                            child: Transform.translate(
                              offset: Offset(0, -value.get<double>(_P.pageScroll)),
                              child: _buildContentColumn(gap, constraints, value),
                            ),
                          ),
                        ))
                  ],
  ...
  Container _buildContentColumn(double gap, BoxConstraints constraints, TimelineValue<_P> value) {
    return Container(
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.all(
          Radius.circular(0.5 * gap),
        ),
      ),
    );
  }

  Container _buildContentContainer(double gap, {Color? color, double heightScale = 1.0}) {
    return Container(
      margin: EdgeInsets.only(bottom: gap),
...
TimelineTween<_P> _createTween(double size, double gap) {
  ...
  final scroll1 = tween.addScene(begin: 500.milliseconds, duration: 900.milliseconds).animate(
        _P.pageScroll,
        tween: 0.0.tweenTo(320.0).curved(Curves.easeOut).curved(Curves.easeOut).curved(Curves.easeOut),
      );

  final scroll2 = scroll1.addSubsequentScene(duration: 600.milliseconds).animate(
        _P.pageScroll,
        tween: 320.0.tweenTo(400.0).curved(Curves.easeOut).curved(Curves.easeOut).curved(Curves.easeOut),
      );
  ...
  final clickOut = clickIn.addSubsequentScene(duration: 100.milliseconds).animate(
        _P.contentColor,
        tween: grey3.tweenTo(grey2),
      );
