...
class LayoutC extends StatelessWidget {
  ...
  const LayoutC({Key? key, this.start = 0.0}) : super(key: key);
...
TimelineTween<_P> _createTween(double size, double gap) {
  ...
  final clickOut =
      clickIn.addSubsequentScene(duration: 100.milliseconds).animate(_P.color1, tween: (grey3).tweenTo(grey2));
  ...
  final growSide = clickOut
      ...
      .animate(_P.top1, tween: (0.4 * size).tweenTo(0.0), shiftBegin: 300.milliseconds)
      .animate(_P.height1, tween: (0.25 * size).tweenTo(size), shiftBegin: 300.milliseconds)
      .animate(_P.color1, tween: (grey2).tweenTo(grey1), shiftBegin: 500.milliseconds);
  ...
  final openPage = clickOut
      ...
      .animate(_P.opacity2, tween: (0.0).tweenTo(1.0), shiftBegin: 300.milliseconds, shiftEnd: 200.milliseconds)
      .animate(_P.color2, tween: (grey4).tweenTo(grey3));

  final scroll = openPage.addSubsequentScene(delay: 500.milliseconds, duration: 900.milliseconds).animate(_P.scroll2,
      tween: (0.0).tweenTo(300.0).curved(Curves.easeOut).curved(Curves.easeOut).curved(Curves.easeOut));

  openPage.addSubsequentScene(duration: 1.milliseconds).animate(_P.push, tween: (size).tweenTo(0.0));
  ...
  scroll
      ...
      .animate(_P.opacity2, tween: (1.0).tweenTo(0.0), shiftEnd: -300.milliseconds)
